...
const CMD = 'json2dart';
const CFGFILE = '.json2dart';
const HOMEPAGE = "https://www.npmjs.com/package/json-to-dart";
...
选项: 
  ...
  -n, --name NAME             生成类的名称.
      --plugin ID/NAME/ALIAS  插件主要用于转换其他工具生成的Dart文件，可以使用ID/Name/Alias来选择插件
      --plugins [FORMAT]      列出所有支持的插件，支持变量：%id、%name、%homepage、%alias
...
举例:
  1. 按照提示从标准输入获取数据生成:
    ${CMD} --name ClassName 
  2. 从JSON文件获取数据生成:
    ${CMD} --name ClassName --from data.json
  3. 从HTTP请求获取数据生成:
    ${CMD} --name ClassName --from http://url/of/data.json
  4. 重新构建已生成的Dart文件
    ${CMD} --rebuild lib/models/class_name.dart

在当前工作目录的\`${CFGFILE}\`可以配置一些通用选项.
{
  "help": "${HOMEPAGE}",
...
Options: 
  ...
  -f, --force                Force save to file, even if the file already exists.
      ...
      --init                 Initialize the project config file \`${CFGFILE}\`
  ...
  -n, --name CLASS NAME      The class name to be generated of top object.
      --plugin ID/NAME/ALIAS The plugin is mainly used to convert Dart files generated by other tools,
                             You can use ID, name, alias to select plugins.
      --plugins              List all supported plugins, variables: %id, %name, %homepage, %alias
...
The file \`${CFGFILE}\` in the work directory provides common options.
{
  "help": "${HOMEPAGE}",
...
function main () {
  ...
  if (ifUndefined(getArg('version'), getShortArg('v'))) {
    ...
    return help();
  } else if (getArg('plugins')) {
    printPlugins(getArg('plugins'));
    ...
    if (force) {
      let cfg0 = readJsonOrFile(CFGFILE);
      Object.assign(cfg, cfg0);
      fs.writeFileSync(CFGFILE, JSON.stringify(cfg, null, 2));
    } else {
      fs.writeFileSync(CFGFILE, JSON.stringify(cfg, null, 2), { flag: 'wx' });
    }
    console.log('config file gnerated: ' + path.resolve(CFGFILE));
    return process.exit(0);
  }
  ...
  let restorer = selfRestorer;
  if (plugin) {
    restorer = getPlugin(plugin);
    if (!restorer) {
  ...
  if (restoreFrom) {
    return restorer(restoreFrom).then(function (data) {
      if (verbose) {
  ...
  const cfg = readJsonOrFile(CFGFILE);
  const defaults = {
    formatter: 'dartfmt',
    arrayItemSuffix: 'Item',
    concat: true,
  ...
  function normalize (data, from, fromOption, cmd, args) {
    ...
    if (!cfg.help) {
      codes.push(`// Usage see ${HOMEPAGE}`);
    }
    codes.push('');
    codes.push(data.code);
    return formatCode(codes.join('\n'), formatter).then(function (code) {
      if (dryRun) {
        data.map.file = file;
        if (verbose) {
          try {
  ...
  if (rebuild) {
    return restorer(rebuild).then(function (data) {
  ...
  function fetchConfig () {
    if (config === true && name || /\.dart$/.test(config)) {
      return restorer(getOutputFile(config === true ? name : config)).then(function (data) {
        delete data.data;
